...
class TestMemobase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_long_term_memory.search.return_value = [
            (0.8, {"id": 1, "text": "Test memory",
                  "meta_data": {"user_id": 123}}),
            (0.7, {"id": 2, "text": "Another test",
                  "meta_data": {"user_id": 123}})
        ]
        self.mock_long_term_memory.get_recent_memories.return_value = [
            {"id": 1, "text": "Test memory", "meta_data": {"user_id": 123},
             "created_at": time.time()},
            {"id": 2, "text": "Another test", "meta_data": {"user_id": 123},
             "created_at": time.time()}
        ]
    ...
    async def test_search(self, mock_to_thread):
        ...
        # Search for content
        results = await self.memobase.search(
            query="test",
            limit=5,
            user_id=123
        )
