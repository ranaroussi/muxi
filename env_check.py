...
def check_env_keys():
    ...
    print(f"OPENAI_API_KEY before loading: {openai_key_before[:10]}...{openai_key_before[-5:] if len(openai_key_before) > 15 else ''}")
    ...
    print(f"OPENAI_API_KEY after loading: {openai_key[:10]}...{openai_key[-5:] if len(openai_key) > 15 else ''}")

    # Check OpenAI client
    client = openai.Client(api_key=openai_key)
    client_key = client.api_key
    print(f"OpenAI client API key: {client_key[:10]}...{client_key[-5:] if len(client_key) > 15 else ''}")
    ...
    if os.path.exists(env_path):
        ...
        for line in env_content:
            if line.strip().startswith("OPENAI_API_KEY="):
                key = line.strip().split("=", 1)[1]
                print(f"OPENAI_API_KEY in .env file: {key[:10]}...{key[-5:] if len(key) > 15 else ''}")
                break
